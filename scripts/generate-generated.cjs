...
function loadLocalActrType(configPath) {
  if (!fs.existsSync(configPath)) return null;
  const config = readToml(configPath);
  const pkg = config.package || {};
  if (pkg.actr_type && pkg.actr_type.manufacturer && pkg.actr_type.name) {
    return {
      manufacturer: String(pkg.actr_type.manufacturer),
      name: String(pkg.actr_type.name),
    };
  }
  if (pkg.actor && pkg.actor.manufacturer && pkg.actor.name) {
    return {
      manufacturer: String(pkg.actor.manufacturer),
      name: String(pkg.actor.name),
    };
  }
  return null;
}
...
function renderPbTs(pkg, enums, types, typeNameMap, protoLabel) {
  ...
  for (const type of types) {
    ...
    if (lengthDelimited.length > 0) {
      ...
      lines.push('        const value: Buffer = buffer.subarray(offset, end);');
  ...
  lines.push('function encodeVarint(value: number): Buffer {');
  ...
  lines.push('}');
  lines.push('');
  lines.push('function encodeVarintBigint(value: bigint): Buffer {');
  ...
  lines.push('}');
  lines.push('');
  lines.push('function decodeVarint(buffer: Buffer, offset: number): { value: bigint; length: number } {');
  ...
  lines.push('}');
  lines.push('');
  lines.push('function varintToNumber(value: bigint, label: string): number {');
  ...
  lines.push('}');
  lines.push('');
  lines.push('function encodeZigZag32(value: number): number {');
  lines.push('  return (value << 1) ^ (value >> 31);');
  lines.push('}');
  lines.push('');
  lines.push('function encodeZigZag64(value: bigint): bigint {');
  lines.push('  const v = BigInt(value);');
  lines.push('  return (v << 1n) ^ (v >> 63n);');
  lines.push('}');
  lines.push('');
  lines.push('function decodeZigZag32(value: number): number {');
  lines.push('  return (value >>> 1) ^ -(value & 1);');
  lines.push('}');
  lines.push('');
  lines.push('function decodeZigZag64(value: bigint): bigint {');
  lines.push('  return (value >> 1n) ^ (-(value & 1n));');
  lines.push('}');
  lines.push('');
  lines.push('function encodeFixed32(value: number): Buffer {');
  lines.push('  const buf = Buffer.alloc(4);');
  lines.push('  buf.writeUInt32LE(value >>> 0, 0);');
  lines.push('  return buf;');
  lines.push('}');
  lines.push('');
  lines.push('function encodeFixed64(value: bigint): Buffer {');
  ...
  lines.push('}');
  lines.push('');
  lines.push('function encodeFloat32(value: number): Buffer {');
  lines.push('  const buf = Buffer.alloc(4);');
  lines.push('  buf.writeFloatLE(value, 0);');
  lines.push('  return buf;');
  lines.push('}');
  lines.push('');
  lines.push('function encodeFloat64(value: number): Buffer {');
  lines.push('  const buf = Buffer.alloc(8);');
  lines.push('  buf.writeDoubleLE(value, 0);');
  lines.push('  return buf;');
  lines.push('}');
  lines.push('');
  lines.push('function readFixed32(buffer: Buffer): number {');
  lines.push('  return buffer.readUInt32LE(0);');
  lines.push('}');
  lines.push('');
  lines.push('function readFixed64(buffer: Buffer): bigint {');
  lines.push('  return buffer.readBigUInt64LE(0);');
  lines.push('}');
  lines.push('');
  lines.push('function readFloat32(buffer: Buffer): number {');
  lines.push('  return buffer.readFloatLE(0);');
  lines.push('}');
  lines.push('');
  lines.push('function readFloat64(buffer: Buffer): number {');
...
function renderClientTs(pkg, services) {
  const routeKeys = [];
...
function renderClientJs(pkg, services) {
  const routeKeys = [];
...
function renderLocalActorTs(localServices, typeNameMap, packageMethodDuplicates, distImport, routeEntries) {
  ...
  lines.push('// Generated by actr-ts-codegen.');
  lines.push('');
  lines.push(`import { Context, RpcEnvelope } from '${distImport}';`);
  lines.push(`import type { PayloadType } from '${distImport}';`);

  const localRoutes = [];
  const routeKeyConstants = [];
  ...
  for (const entry of localServices) {
    ...
    for (const method of entry.service.methodsArray) {
      ...
      routeKeyConstants.push({ constName, routeKey });
  ...
  for (const entry of routeEntries) {
    if (entry.routeKey) routeKeyConstants.push({ constName: entry.constName, routeKey: entry.routeKey });
  }
  ...
  for (const { constName, routeKey } of routeKeyConstants) {
    lines.push(`export const ${constName} = '${routeKey}';`);
  }
  if (routeKeyConstants.length > 0) lines.push('');
...
function renderLocalActorJs(routeEntries, distImport) {
  ...
  if (routeEntries.length > 0) lines.push('');
...
async function main() {
  ...
  const localType = loadLocalActrType(configPath);
  ...
  for (const [pkg, data] of packages.entries()) {
    ...
    if (data.services.length > 0) {
      ...
      removeFileIfExists(clientTsPath);
      ...
      for (const service of data.services) {
        ...
        for (const method of service.methodsArray) {
          ...
          const isLocal =
            localType &&
            localType.manufacturer === targetType.manufacturer &&
            localType.name === targetType.name;
          if (!isLocal) {
            routeEntries.push({ constName, targetType, importFrom, requireFrom, fileBase, routeKey });
          }
        }
      }
    }
