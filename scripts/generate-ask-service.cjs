...
function usage() {
  const message = [
    ...
    '',
    'Example:',
    '  node scripts/generate-ask-service.cjs --proto askaway-proto/ask-service/ask.proto --out-dir src --service AskService',
  ].join('\n');
...
function renderRuntime(serviceInfo, typeNameMap) {
  ...
  lines.push('');
  lines.push("import type { Context, RpcEnvelope, Workload } from '@actor-rtc/actr';");
  lines.push('');
  ...
  lines.push(`import { ${routeImports.join(', ')} } from './generated/local.actor.js';`);
  ...
  for (const method of service.methodsArray) {
    ...
    lines.push(`  ${methodName}: (request: ${reqType}, ctx: Context) => Promise<${resType}>;`);
  }
  lines.push('}');
  lines.push('');
  ...
  lines.push('');
  lines.push('  async onStart(_ctx: Context): Promise<void> {');
  lines.push(`    console.log('${serviceName} workload started');`);
  lines.push('  }');
  lines.push('');
  lines.push('  async onStop(_ctx: Context): Promise<void> {');
  lines.push(`    console.log('${serviceName} workload stopped');`);
  lines.push('  }');
  lines.push('');
  lines.push('  async dispatch(ctx: Context, envelope: RpcEnvelope): Promise<Buffer> {');
...
function renderScaffold(serviceInfo, typeNameMap) {
  ...
  lines.push('');
  lines.push("import type { Context } from '@actor-rtc/actr';");
  lines.push('');
  ...
  for (const method of service.methodsArray) {
    ...
    lines.push(`  async ${methodName}(request: ${reqType}, _ctx: Context): Promise<${resType}> {`);
